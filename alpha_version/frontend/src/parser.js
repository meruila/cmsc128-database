    ...
    for (let j=0; j<fileList.length; j++){
        ...
        if (parseResult.success){
            ...
            if (!formatted.success)
                errList.push({type:"Formatting Error:", msg:formatted.err, filename:readItem.filename})
            else
                formattedList.push({filename:readItem.filename, data:formatted.data})
        } else {
            errList.push({type:"Parsing Error:", msg:parseResult.err, filename:readItem.filename})
        }
    }
