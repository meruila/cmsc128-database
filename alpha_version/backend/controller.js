       ...
       return jwt.verify(
         ...
         async (err, tokenPayload) => {
           ...
           if (isExisting){
               ...
               if(isAdmin){
                 return {success: true, note: "User is admin", tokenEmail: tokenPayload._email};
               }else {
                // console.log("Is NOT Admin");
                 return {success: false, note: "User is not admin"}; 
               }
           }else{
 ...
 exports.addStudentRecord = async (req,res) => {  
    try{
        ...
        if(isSuccess) {
            ...
            const isSaved = (saveStatus.length > 0) ? true : false;
            if (isSaved) {
                res.status(200);
                return res.send({success: true, note: "At least one record was saved.", savedList: saveStatus});
            } else {
                res.status(400);
                return res.send({success: false, note: "No records were saved", savedList: saveStatus});
            }
        }else {
            res.status(403);
            return res.send({success: false, note: "User is not logged in.", savedList: []});
        }
    }catch (err) {
        res.status(500);
        return res.send({success: false, note: "Something went wrong", savedList: err.savedList});
    }    
} // DONE!!!
...
exports.viewStudentRecord = async (req,res) => {
    try{
        ...
        // check if request came from logged in user
        if(isSuccess){
            const rec = await dbRecord.viewStudentRecord(req.body.studentNo);
...
exports.updateStudentRecord = async (req,res) => {
    try{
        ...
        if(isSuccess){
            ...
            const updateStatus = await dbRecord.editStudentRecord(newDetails);
            if(updateStatus){
                res.status(200);
...
exports.deleteStudentRecord = async (req,res) => {
    try {
        ...
        // check if request came from logged in user
        if(isSuccess){
            const deleteStatus = await dbRecord.delStudentRecord(req.body.studentNo);
            if(deleteStatus){
                res.status(200);
...
exports.viewSummary = async (req,res) => {
    try{
        ...
        // check if request came from logged in user
        if(isSuccess){
            const giveStatus = await dbRecord.showSummary();
            res.status(200);
